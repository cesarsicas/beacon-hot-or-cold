...
import static br.com.cesarsicas.beaconhotorcold.Core.Main.Constants.MAX_EXPECTED_TX_POWER;
import static br.com.cesarsicas.beaconhotorcold.Core.Main.Constants.MIN_EXPECTED_TX_POWER;
...
public class UidValidator {
  ...
  public static void validate(String deviceAddress, byte[] serviceData, Beacon beacon) {
    ...
    beacon.uidStatus.uidValue = BeaconUtils.toHexString(uidBytes);
    if (BeaconUtils.isZeroed(uidBytes)) {
    ...
    if (beacon.uidServiceData == null) {
      ...
      if (!Arrays.equals(uidBytes, previousUidBytes)) {
        String err = String.format("UID should be invariant.\nLast: %s\nthis: %s",
            BeaconUtils.toHexString(previousUidBytes),
            BeaconUtils.toHexString(uidBytes));
        beacon.uidStatus.errUid = err;
        logDeviceError(deviceAddress, err);
        beacon.uidServiceData = serviceData.clone();
      }
    ...
    if (rfu[0] != 0x00 || rfu[1] != 0x00) {
      String err = "Expected UID RFU bytes to be 0x00, were " + BeaconUtils.toHexString(rfu);
