...
public class UrlValidator {
  ...
  public static final String TAG = UrlValidator.class.getSimpleName();
  ...
  public static void validate(String deviceAddress, byte[] serviceData, Beacon beacon) {
    ...
    if (BeaconUtils.isZeroed(urlBytes)) {
      String err = "URL bytes are all 0x00";
      beacon.urlStatus.urlNotSet = err;
      logDeviceError(deviceAddress, err);
    }

    beacon.urlStatus.urlValue = EddystoneUrlUtils.decodeUrl(serviceData);
  }

  public static void logDeviceError(String deviceAddress, String err) {
